...
* 是否可以使用清除所有內容和設定（重置），建議學生機**設定為限制**，避免學生自己手動重置 iPad
  * 👨‍🏫 教師機建議設定：不設定
  * 👩‍🎓 學生機建議設定：⛔ 限制
...
* 是否可以登入 Apple ID，如果有使用管理式 Apple ID 要允許，沒有建議**學生機設定為限制**
  * 👨‍🏫 教師機建議設定：✅ 允許
  * 👩‍🎓 學生機建議設定：⛔ 限制

## 修改藍牙設定

* 是否可以修改藍牙設定，如果限制會沒辦法新增藍牙裝置（鍵盤、觸控筆...等），建議不設定
  * 👨‍🏫 教師機建議設定：✅ 允許
  * 👩‍🎓 學生機建議設定：✅ 允許
...
* 是否可以使用輕巧 App，iPad 無 NFC 功能，可略過
